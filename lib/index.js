...
NwBuilder.prototype.handleMacApp = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if(typeof self.options.macPlist === 'string') {
            ...
            // Also Update Helper Plist
            var matches = Glob(path.resolve(platform.releasePath, self.options.appName+'.app', 'Contents', 'Versions', '*', 'nwjs Helper.app', 'Contents', 'Info.plist'));
            if (matches.length > 0) {
                var HelperPlistPath = matches[0];
                allDone.push(Utils.editHelperPlist(HelperPlistPath, HelperPlistPath, plistOptions));
            }
        }
    });
